...
def mse(outputs, targets):
    ...
    """
    # Flatten tensors
    outputs = outputs.reshape(outputs.nelement())
    targets = targets.reshape(targets.nelement())

    # Check dim
    if outputs.size() != targets.size():
        raise ValueError(u"Ouputs and targets tensors don't have the same number of elements")
    # end if
