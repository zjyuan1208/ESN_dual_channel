...
mean_errors_feat_0 = [np.mean(np.abs(np.array(pred) - np.array(target)) * 6) for pred, target in zip(pred_peaks_feat_0_pure_list, target_peaks_feat_0_list)]
std_dev_feat_0 = [np.std(np.abs(np.array(pred) - np.array(target)) * 6) for pred, target in zip(pred_peaks_feat_0_pure_list, target_peaks_feat_0_list)]

mean_errors_feat_1 = [np.mean(np.abs(np.array(pred) - np.array(target)) * 6) for pred, target in zip(pred_peaks_feat_1_pure_list, target_peaks_feat_1_list)]
std_dev_feat_1 = [np.std(np.abs(np.array(pred) - np.array(target)) * 6) for pred, target in zip(pred_peaks_feat_1_pure_list, target_peaks_feat_1_list)]
colors = ['#80221E', '#AD7C59', '#B85C48', '#CABCAB']
# labels = ['1 1 1.2Hz', '1 2 1.2Hz', '1 3 1.2Hz', '2 2 1.2Hz', '2 3 1.2Hz', '3 3 1.2Hz', '1 1 2.4Hz', '1 2 2.4Hz', '1 3 2.4Hz', '2 2 2.4Hz', '2 3 2.4Hz', '3 3 2.4Hz']
labels = ['1 1 1.2Hz', '1 2 1.2Hz', '1 3 1.2Hz', '2 2 1.2Hz', '1 1 2.4Hz', '1 2 2.4Hz', '1 3 2.4Hz', '2 2 2.4Hz', '2 3 2.4Hz', '3 3 2.4Hz']
# Plotting
...
bar1 = ax.bar(index, mean_errors_feat_0, bar_width, label='Feature 0', yerr=std_dev_feat_0, capsize=3, color=colors[0])
...
ax.set_ylabel('Time Offset Ratio (ms)')
...
ax.legend()
if save_state:
    plt.savefig(f'{save_path}/prediction_error_two_channel.png', dpi=300)
plt.show()
